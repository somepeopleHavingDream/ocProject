...
public class BeanUtil {
    ...
    public static String columnToField(String str) {
        ...
        if (arr.length > 1) {
            StringBuilder rstStr = new StringBuilder(arr[0]);
            for (int i = 1; i < arr.length; i++) {
                rstStr.append(arr[i].substring(0, 1).toUpperCase()).append(arr[i].substring(1));
            }
    ...
    public static String columnToField2(String str) {
        String[] arr = str.split("_");
        if (arr.length > 1) {
            StringBuilder rstStr = new StringBuilder(arr[0].substring(0, 1).toUpperCase() + arr[0].substring(1));
            for (int i = 1; i < arr.length; i++) {
                rstStr.append(arr[i].substring(0, 1).toUpperCase()).append(arr[i].substring(1));
            }
            return rstStr.toString();
    ...
    public static String fieldToColumn(String str) {
        ...
        for (char aChar : chars) {
            if (aChar > 64 && aChar < 94) {
                rstStr.append(("_" + aChar).toLowerCase());
            } else {
                rstStr.append(aChar);
            }
        }
